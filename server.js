...
app.get("/order_by_id", async (req, res) => {
  ...
  Promise.all([queryOrderById, queryProductsByOrderID, queryClientById]).then(
    (results) => {
      res.send(results);
    }
  );
...
app.post("/neworder", async (req, res) => {
  ...
  if (isNewClient) {
    ...
    const queryAddNewOrder = await addNewOrder(
      dataDetails,
      queryAddNewClient,
      totalCostOfProducts,
      currentDate
    );
    const queryAddMultipleProducts = await addMultipleProducts(
      queryAddNewOrder,
      productsData
    );
    Promise.all([
...
app.post("/updateorder", async (req, res) => {
  ...
  const queryUpdateClientById = await updateClientById(
    clientData,
    orderData.client_id
  );
  const queryUpdateOrderById = await updateOrderById(
    totalCostOfProducts,
    orderData.status,
    orderId
  );
  ...
  if (deletedIds.length > 0) {
    ...
    Promise.all([
      queryUpdateClientById,
      queryUpdateOrderById,
      queryAddMultipleProducts,
      queryDeleteProductsById,
    ]).then(() => {
      res.send("success");
    });
  } else {
...
app.get("/client_by_id", async (req, res) => {
  ...
  const queryProductsByOrderId = await getProductsByMultipleOrderId(
...
app.post("/newuser", async (req, res) => {
  const userDetails = req.body.userDetails;
  const plainTextPassword = userDetails.password; // Use plaintext password
  const queryCreateNewUser = await createNewUser(
    userDetails,
    plainTextPassword
